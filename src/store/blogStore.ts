...
export const useBlogStore = create<BlogStore>((set, get) => ({
  ...
  fetchPosts: async () => {
    ...
    try {
      ...
      if (error) {
        ...
        set({ error: '방명록을 불러오는 중 오류가 발생했습니다.', loading: false });
        return;
      }
  ...
  fetchPost: async (id: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('Supabase error:', error);
        set({ error: '방명록을 찾을 수 없습니다.', loading: false });
        return;
      }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42501') {
          ...
          set({ success: '방명록이 생성되었습니다! (로컬 저장 - RLS 정책으로 인해 서버 저장 실패)' });
          return;
        }
        set({ error: '방명록 생성 중 오류가 발생했습니다.', loading: false });
        return;
      }
      ...
      if (data) {
        ...
        set({ success: '방명록이 성공적으로 생성되었습니다!' });
      }
    } catch (error) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42501') {
          ...
          set({ success: '방명록이 수정되었습니다! (로컬 저장 - RLS 정책으로 인해 서버 저장 실패)' });
          return;
        }
        set({ error: '방명록 수정 중 오류가 발생했습니다.', loading: false });
        return;
      }
      ...
      if (data) {
        ...
        set({ success: '방명록이 성공적으로 수정되었습니다!' });
      }
    } catch (error) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42501') {
          ...
          set({ success: '방명록이 삭제되었습니다! (로컬 저장 - RLS 정책으로 인해 서버 저장 실패)' });
          return true;
        }
        set({ error: '방명록 삭제 중 오류가 발생했습니다.', loading: false });
        return false;
      }
      ...
      set({ success: '방명록이 성공적으로 삭제되었습니다!' });
      return true;
    } catch (error) {
